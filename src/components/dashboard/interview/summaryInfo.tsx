...
// This type must match the error: it needs 'responses' and 'interview'
type SummaryProps = {
  responses: Response[];
...
function SummaryInfo({ responses, interview }: SummaryProps) {
  ...
  // Correctly mapping video_url to show up in the DataTable
  const prepareTableData = (responses: Response[]): TableData[] => {
    return responses.map((response) => ({
      ...
      video_url: response.video_url || "", // This link will now appear in your list
      callSummary:
  ...
  useEffect(() => {
    ...
    responses.forEach((response) => {
      ...
      const agentTask = response.details?.call_analysis?.agent_task_completion_rating;
      if (agentTask === "Complete" || agentTask === "Partial") {
        completedCount += 1;
      }
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col items-center justify-center gap-1 mx-2 p-3 rounded-2xl bg-slate-50 shadow-md max-w-[360px]">
                <div className="flex flex-row gap-1 font-semibold mb-1 text-[15px] mx-auto text-center">
                  Completion Rate
                  <InfoTooltip content="Success percentage" />
                </div>
