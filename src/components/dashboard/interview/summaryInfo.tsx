...
// Updated prop type to match what the parent page sends
type SummaryProps = {
  responses: Response[];
...
function SummaryInfo({ responses, interview }: SummaryProps) {
  ...
  // Correctly mapping the video_url for the DataTable
  const prepareTableData = (responses: Response[]): TableData[] => {
    return responses.map((response) => ({
      ...
      video_url: response.video_url || "", // Pass the URL to the table
      callSummary:
  ...
  useEffect(() => {
    ...
    responses.forEach((response) => {
      ...
      const completion = response.details?.call_analysis?.call_completion_rating;
      ...
      totalDurationCount += response.duration;
        ...
        <div className="bg-slate-200 rounded-2xl min-h-[120px] p-2 ">
          ...
          <div className="flex flex-row gap-1 my-2 justify-center">
            ...
            <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-4 rounded-2xl bg-slate-50 shadow-md max-w-[360px]">
              ...
              <PieChart
                sx={{ "& .MuiChartsLegend-series text": { fontSize: "0.8rem !important" } }}
                series={[{
                  data: [
            ...
            <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-4 rounded-2xl bg-slate-50 shadow-md">
              <div className="flex flex-row gap-2 text-[15px] font-bold mx-auto mb-1">
                <UserCircleIcon /> Candidate Status
              ...
              <PieChart
                ...
                series={[{
                  data: [
                    { id: 0, value: candidateStatusCount[CandidateStatus.SELECTED], label: `Selected (${candidateStatusCount[CandidateStatus.SELECTED]})`, color: "#22c55e" },
                    { id: 1, value: candidateStatusCount[CandidateStatus.POTENTIAL], label: `Potential (${candidateStatusCount[CandidateStatus.POTENTIAL]})`, color: "#eab308" },
                    { id: 2, value: candidateStatusCount[CandidateStatus.NOT_SELECTED], label: `Not Selected (${candidateStatusCount[CandidateStatus.NOT_SELECTED]})`, color: "#eb4444" },
                    { id: 3, value: candidateStatusCount[CandidateStatus.NO_STATUS], label: `No Status (${candidateStatusCount[CandidateStatus.NO_STATUS]})`, color: "#9ca3af" },
                  ],
                  highlightScope: { faded: "global", highlighted: "item" },
                  faded: { innerRadius: 10, additionalRadius: -10, color: "gray" },
                }]}
